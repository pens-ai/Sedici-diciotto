...
// Sync a single property's iCal - creates CalendarBlocks (not Bookings)
async function syncPropertyIcal(property) {
  const iCalUrls = property.iCalUrls || [];

  if (iCalUrls.length === 0) {
    return { propertyId: property.id, imported: 0, updated: 0, errors: [] };
  }
  ...
  return { propertyId: property.id, propertyName: property.name, imported, updated, errors };
}
...
async function syncAllProperties() {
  ...
  try {
    ...
    for (const property of propertiesWithUrls) {
      ...
      console.log(`[iCal Sync] ${property.name}: ${result.imported} new blocks, ${result.updated} updated`);
    }

    console.log(`[iCal Sync] Complete: ${totalImported} new, ${totalUpdated} updated`);
